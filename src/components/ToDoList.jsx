import React from 'react'; // Non hai bisogno di useState/useEffect qui
            ...
            <ul className="w-full max-w-md space-y-4">
                ...
                {tasks.map(item => (
                    <li key={item.id} className="flex items-center justify-between bg-gray-100 p-4 rounded-lg shadow-sm">
                        {/* Modifica qui: aggiunta di flex-shrink-1 e la classe custom 'text-wrap-fix' */}
                        <span className={`text-lg text-gray-800 flex-grow flex-shrink-1 min-w-0 text-wrap-fix ${item.completed ? 'line-through text-gray-500' : ''}`}>
                            {item.title}
                        </span>
